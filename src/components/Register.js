      ...
      await signup(data.email, data.password)
        ...
        .catch((err) => {
          if (err.code === "auth/email-already-in-use") {
            toast.error(
              <span className="alertText">Email address already exists!</span>,
              {
                position: "top-center",
                autoClose: 5000,
                hideProgressBar: false,
                ...
                <select className="utf-chosen-select-single-item utf-with-border" title="Single User"> 
                  ...
                  <option>Platinum</option>	
                    ...
                    <div className="form-row form-row-wide">
                      ...
                      {errors?.firstname?.type === "required" && (
                        <div className="formError">firstname is required</div>
                      )}
                    </div>
                    <div className="form-row form-row-wide">
                      <input
                      ...
                      {errors?.secondname?.type === "required" && (
                        <div className="formError">second name is required</div>
                      )}
                    </div>
                    <div className="form-row form-row-wide">
                      <input
                      ...
                      {errors?.email?.type === "required" && (
                        <div className="formError">email is required</div>
                      )}
                      {errors?.email?.type === "pattern" && (
                        <div className="formError">invalid email address</div>
                      )}
                    </div>
                    <div className="form-row form-row-wide">
                      <input
                    ...
                    <div
                      className="form-row checkbox margin-top-10 margin-bottom-10"
                      onClick={() => {
                        setAgree(!agree);
                        setAgreeError(false);
                      }}
                    >
                      ...
                      <label htmlFor="two-step0">
                        <span
                          className={`checkbox-icon ${agree && "markcheck"}`}
                        />{" "}
                    ...
                    {agreeError && (
                      <div className="formError">agree to continue</div>
                    )}
                    <input
                      type="submit"
                  ...
                  <div className="utf-social-login-buttons-block">
                    <button
                      ...
                      className="button social-login via-twitter"
                    >
                      <i className="icon-brand-facebook-f" /> Facebook
                    </button>
                    <button
                      onClick={handleGoogle}
                      className="button social-login via-gplus"
                    >
                      <i className="icon-brand-google-plus-g" /> Google+
                    </button>
                    <button
                      onClick={handleTwitter}
                      className="button social-login via-facebook"
                    >
